...
class Functionality:
    ...
    def generate_data(self, transaction_details=transaction_details):
        data = [
            transaction_details.transaction_dict[x]
            for x in transaction_details.transaction_dict.keys()
        ]
...
class MainScreen(QDialog, Functionality):
    ...
    def __init__(self):
        ...
        self.CalculateOrder.clicked.connect(
            self.CalculateOrderTrigger
    ...
    def save_input(self):
        try:
            ...
            if (item_name != "") & (item_qty != "") & (item_price != ""):
                transaction_details.add_item(
                    item_name=item_name, item_amount=item_qty, price_per_item=item_price
                )
                self.ItemNameBox.clear()
                self.ItemQtyBox.clear()
                self.ItemPriceBox.clear()
                self.Status.setText(
                    f"Berhasil Menambahkan Item {transaction_details.transaction_dict.get(item_name)}"
                )
    ...
    def save_item_name(self):
        try:
            ...
            if (item_name != "") & (new_name != ""):
                transaction_details.update_item_name(
                    old_name=item_name, new_name=new_name
                )
                self.Status.setText(
                    "Berhasil Mengubah Nama Item, dari {} menjadi {}".format(
                        item_name, new_name
                    )
                )
    ...
    def save_item_qty(self):
        try:
            ...
            if (item_name != "") & (new_qty != ""):
                ...
                transaction_details.update_item_qty(
                    item_name=item_name, new_qty=new_qty
                )
                self.model = DictionaryTableModel(*self.generate_data())
                self.ItemTable.setModel(self.model)
    ...
    def save_item_price(self):
        ...
        try:
            ...
            if (item_name != "") & (new_price != ""):
                ...
                transaction_details.update_item_price(
                    item_name=item_name, new_price=new_price
                )
                self.model = DictionaryTableModel(*self.generate_data())
                self.ItemTable.setModel(self.model)
...
class CalculateTotalOrderScreen(QDialog, Functionality):
    ...
    def order_recap(self):
        ...
        text = "Total Pembayaran : Rp{:,.2f}, Selamat anda mendapatkan diskon sebesar {}%".format(
            price, round(discount * 100, 2)
        )
